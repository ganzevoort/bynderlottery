...
export default function MyBallotsPage() {
  ...
  const onSubmitPurchase = async (data: BallotPurchaseForm) => {
    ...
    try {
      ...
      const message =
        errorResponse.response?.data?.message || 'Failed to purchase ballots';
      toast.error(message);
    } finally {
  ...
  const onSubmitAssignment = async (data: BallotAssignmentForm) => {
    ...
    try {
      ...
      for (const ballot of ballotsToAssign) {
        await LotteryService.assignBallot(ballot.id, {
      ...
      const errorResponse = error as ApiErrorResponse;
      console.error('Assignment error:', errorResponse);
      const message =
        errorResponse.response?.data?.error || 'Failed to assign ballots';
      toast.error(message);
    } finally {
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">
          Manage your lottery tickets, purchase new ones, and assign them to
          draws.
        </p>
      </div>
      ...
      <div className="card card-hover">
        ...
        {expandedSections.purchase && (
          <div className="px-6 pb-6">
            <form
              onSubmit={handleSubmit(onSubmitPurchase)}
              className="space-y-4"
            >
        ...
        >
          <div className="flex items-center">
            ...
            <h2
              ...
              data-testid="unassigned-ballots-title"
            >
          ...
          <div className="px-6 pb-6">
            {ballots?.unassigned_ballots.length === 0 ? (
              <p className="text-gray-600 py-4">
                No unassigned ballots. Purchase some to get started!
              </p>
            ) : (
                ...
                <div className="card card-hover p-4">
                  <div className="mb-4">
                    <h3 className="text-sm font-medium text-yellow-800">
                      {ballots?.unassigned_ballots.length || 0} Unassigned
                      Ballot
                      {(ballots?.unassigned_ballots.length || 0) !== 1
                        ? 's'
                        : ''}
                    </h3>
                    <p className="text-sm text-yellow-700 mt-1">
                      Assign your ballots to draws to participate in the
                      lottery.
                    </p>
                  </div>
                  ...
                  {/* Assignment Form */}
                  <form
                    onSubmit={assignmentForm.handleSubmit(onSubmitAssignment)}
                    className="space-y-4"
                  >
                      ...
                      <div className="md:col-span-1">
                        ...
                        <input
                          {...assignmentForm.register('quantity', {
                            ...
                            min: { value: 1, message: 'Minimum 1 ballot' },
                            max: {
                              value: ballots?.unassigned_ballots.length || 1,
                              message: `Maximum ${ballots?.unassigned_ballots.length || 1} ballots`,
                            },
                          })}
                          type="number"
                          min="1"
      ...
      <div className="card card-hover">
        ...
        >
          <div className="flex items-center">
            ...
            <h2
              className="text-lg font-semibold text-gray-900"
              data-testid="assigned-ballots-title"
            >
              Assigned Ballots (Open Draws) (
              {ballots?.assigned_ballots.filter(
                (ballot) => ballot.draw && !ballot.draw.closed
              ).length || 0}
              )
            </h2>
          </div>
          ...
          <div className="px-6 pb-6">
            {(() => {
              ...
              if (openDrawBallots.length === 0) {
                return (
                  <p className="text-gray-600 py-4">
                    No ballots assigned to open draws. Assign your ballots to
                    participate!
                  </p>
                );
              }
              ...
              const drawGroups = openDrawBallots.reduce(
                (groups, ballot) => {
                ...
                {} as Record<number, { draw: Draw; ballots: Ballot[] }>
              );

              const sortedDraws = Object.values(drawGroups).sort(
                (a, b) =>
                  new Date(a.draw.date).getTime() -
                  new Date(b.draw.date).getTime()
              );
              ...
              return (
                <div className="space-y-4">
                  {sortedDraws.map((drawGroup) => (
                    <div
                      key={drawGroup.draw.id}
                      className="card card-hover p-4"
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="text-sm font-medium text-gray-900">
                          <div>
                            {drawGroup.draw.drawtype.name} -{' '}
                            {formatDate(drawGroup.draw.date)}
                          </div>
                          <div className="text-green-600 font-medium">
                            Participating with {drawGroup.ballots.length}{' '}
                            ballot
                            {drawGroup.ballots.length !== 1 ? 's' : ''}
                          </div>
                        </div>
                      </div>
                    </div>
      ...
      <div className="card card-hover">
        ...
        >
          <div className="flex items-center">
            ...
            <h2
              className="text-lg font-semibold text-gray-900"
              data-testid="winning-ballots-title"
            >
              Winning Ballots (
              {ballots?.assigned_ballots.filter((ballot) => ballot.prize)
                .length || 0}
              )
            </h2>
          </div>
          ...
          <div className="px-6 pb-6">
            {(() => {
              ...
              if (winningBallots.length === 0) {
                return (
                  <p className="text-gray-600 py-4">
                    No winning ballots yet. Keep participating to win prizes!
                  </p>
                );
              }

              // Group winning ballots by draw and sort by draw date (recent first)
              const drawGroups = winningBallots.reduce(
                (groups, ballot) => {
                  const drawId = ballot.draw!.id;
                  if (!groups[drawId]) {
                    groups[drawId] = {
                        ...
                        {drawGroup.ballots.map((ballot) => (
                          ...
                          >
                            <span className="font-medium text-green-600">
                              {ballot.prize?.name} -{' '}
                              {formatCurrency(ballot.prize?.amount || 0)}
                            </span>
                          </div>
                        ))}
                      </div>
