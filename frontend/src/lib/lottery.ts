import api from './api';
import { Draw, UserBallots, UserWinnings, LotteryStats, BallotPurchaseForm, BallotAssignmentForm } from './types';
...
export class LotteryService {
    ...
    static async purchaseBallots(data: BallotPurchaseForm): Promise<{ message: string; ballots_created: number }> {
        const response = await api.post('/lottery/purchase-ballots/', data);
        return response.data;
    }

    static async assignBallot(ballotId: number, data: BallotAssignmentForm): Promise<{ message: string }> {
        const response = await api.post(`/lottery/ballots/${ballotId}/assign/`, data);
        return response.data;
    }

    static async getBallotDetail(id: number): Promise<{ id: number; draw: Draw | null; prize: any | null }> {
